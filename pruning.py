...
if args.architecture == 'lenet':
    ...
    model = AE(mask=True, input_shape=784).to(device) 
print(model)
...
torch.save(model, f"saves/initial_model_" + args.architecture+ ".ptmodel")
...
train(args.epochs)
torch.save(model, f"saves/model_after_retraining_" + args.architecture +".ptmodel")
accuracy = test_vae() if args.architecture != 'lenet' else test()                                     
